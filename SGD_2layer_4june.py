...
def ann_training(x_df,y_df, hidden_layer1, hidden_layer2, lr, no_of_epoch):
...
hidden_layer2 = 15          # No of neuron in hidden layer-2
lr = 0.01                   # learnining rate
no_of_epoch = 150

# Calling the fuction for training the ANN; pass the training dataset here:
ycap, loss_dict, cc_percentage, wc_percentage, model_param, yu = ann_training(x_df,y_df, hidden_layer1, hidden_layer2, lr, no_of_epoch)

# Calling the fuction for testing the ANN; pass the test dataset here:
ycap, cc_percentage, wc_percentage = ann_testing(x_df, y_df, model_param, yu)


# calling the predict function for forecasting single X dataset; x0 has to be a pandas dataframe only
x0 = [5.4,3.9,1.7,0.4]
y0 = 0
y_pred = ann_predict(x0, model_param, yu)
print("\nForecasting for a single X dataset")
print('actual y:', y0, 'predicted y:', y_pred)
